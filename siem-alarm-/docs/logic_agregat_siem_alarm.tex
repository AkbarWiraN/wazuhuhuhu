...
Destination berbeda dari source. Pada run non-kosong pertama dan rollover tanggal
UTC, concrete daily destination memang belum ada. OpenSearch 2.19 dapat mengembalikan
\code{\_mget} HTTP 200 dengan satu \code{index\_not\_found\_exception} per ID tanpa
field status.\cite{osmget,osmgetsource} Engine menerima kondisi bootstrap ini hanya ketika seluruh item batch
menyebut \code{error.type} dan \code{error.index} yang tepat untuk concrete index
yang diminta. Seluruh state dianggap baru dan two-phase bulk pertama membuat index
melalui template yang telah terpasang. Respons campuran, index berbeda, status
bertentangan, atau flip ada/hilang pada retry batch yang sama tetap gagal tertutup.
Delete eksternal di antara batch tidak dapat dibuat atomik oleh API; operasi itu
tidak didukung dan prosedur stop-before-delete tetap wajib.
...
Engine memilih gagal tertutup ketika menemukan keadaan yang dapat membuat count
salah. Error sebelum bulk pertama tidak menulis destination apa pun. Error pada
batch/fase write yang lebih akhir dapat meninggalkan write deterministik yang sudah
diterima Indexer, tetapi checkpoint tidak maju; ID deterministik, create-only
escalation, dan rerun membuat state konvergen tanpa menggandakan event. Contohnya:
...
Missing daily destination bukan error apabila predicate bootstrap ketat di atas
terpenuhi. Kondisi itu dicatat sebagai INFO karena normal sekali per tanggal UTC.
Jika pesan yang sama muncul untuk index yang sebelumnya ada, operator harus
menganggapnya indikasi delete/race, menghentikan timer, dan menjalankan audit serta
controlled backfill. Membuat ulang index dengan checkpoint lama tidak mengembalikan
bucket historis yang telah dilewati.
...
\code{\_mget index\_not\_found} & Normal pada first nonempty run atau rollover bila log menyebut bootstrap. Jika index sebelumnya ada, hentikan timer dan audit delete/backfill; jangan menghapus checkpoint.\\
\bottomrule
\end{longtable}
...
\bibitem{osmgetsource}
OpenSearch 2.19, \emph{MultiGetResponse.java: failure serialization}.\\
\url{https://github.com/opensearch-project/OpenSearch/blob/2.19/server/src/main/java/org/opensearch/action/get/MultiGetResponse.java}
